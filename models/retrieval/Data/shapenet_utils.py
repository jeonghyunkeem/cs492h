...
SERVER = 'http://shapenet.cs.stanford.edu'
...
def getIdFromPath(path):
    ...
    res = map(os.path.normpath, parts)[::-1]
...
def downloadPNGs(fullId, outDir):
    ...
    for i in range(14):
        ...
        if not os.path.isfile(localFile):
            urllib.urlretrieve(pngUrl, localFile)
            print(pngUrl)
...
def getAlignments():
    ...
    """
    alignmentFile = 'ShapeNetCore-alignments.csv'
    alignURL = ('http://shapenet.cs.stanford.edu/models3d/solr/select?q='
    ...
    if not os.path.isfile(alignmentFile):
        ...
        urllib.urlretrieve(alignURL, alignmentFile)
    alignments = {}
