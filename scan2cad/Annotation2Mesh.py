...
parser.add_argument('--n_count', default=20, help="number of output per one execution")
...
if __name__ == '__main__':
    ...
    for r in JSONHelper.read(filename_json):
        ...
        if os.path.exists(outdir):
            continue
        # ----
        # For memory issue... T.T
        print("\n{}: ".format(i) + id_scan + "---------")
        cnt += 1
        if cnt > int(opt.n_count):
            print(":: {} scans have done ({}/{})".format(opt.n_count, i, 1513))
            break
        # ----

        pathlib.Path(outdir).mkdir(parents=True, exist_ok=True) 
        
        scan_file = params["scannet_all"] + "/" + id_scan + "/" + id_scan + "_vh_clean_2.ply"   # "scannet"
        ...
        for model in r["aligned_models"]:
            ...
            cad_file = params["shapenet_all"] + "/" + catid_cad + "/" + id_cad  + "/models/model_normalized.obj"    # "shapenet"
            # ---
            if os.path.isfile(cad_file) == False:
                print("{} not exist".format(cad_file))
                continue
            # ---

            cad_mesh = pywavefront.Wavefront(cad_file, collect_faces=True, parse=True)
            # if cad_mesh == False:
            #     print("Material Matching Error")
            #     continue
